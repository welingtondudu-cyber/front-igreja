...
* **Estorno com Quantidade**:
  * **Frontend**: Adicionado o campo numérico **"Quantidade"** no modal de estorno de itens, permitindo escolher quantas unidades do produto deseja estornar ao mesmo tempo.
  * **Backend**: O método `estornarItem` foi expandido em [BazarService.java](file:///Users/welingtonsilvacruz/front-igreja/src/main/java/com/suaempresa/gestao/service/BazarService.java) e no endpoint de [BazarController.java](file:///Users/welingtonsilvacruz/front-igreja/src/main/java/com/suaempresa/gestao/controller/BazarController.java) para aceitar o parâmetro `quantidade`. Ele localiza a venda à qual o serial pertence, conta quantos itens com o mesmo ID do produto e status `"VENDIDO"` ainda constam nessa venda específica, e valida que a quantidade a estornar nunca ultrapassa esse limite de itens vendidos (mesmo se o estorno for feito de forma fracionada). Ele reduz proporcionalmente o valor arrecadado da venda.
